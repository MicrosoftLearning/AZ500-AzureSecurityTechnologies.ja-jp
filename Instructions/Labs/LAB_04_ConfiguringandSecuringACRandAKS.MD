...
lab:
  ...
  description: このタスクでは、Dockerfile を作成し、Dockerfile からイメージをビルドして、ACR にイメージをデプロイします。
  duration: 45 minutes
  level: 300
  islab: true
---
...
2. Azure portal から、Azure portal の右上にあるアイコンをクリックして、Cloud Shell を開きます。 メッセージが表示されたら、**[Bash]**、**[ストレージ アカウントは必要ありません]** の順にクリックし、ご自分の*サブスクリプション*を選んでから、**[適用]** をクリックします。
...
6. Bash セッションで次のコマンドを実行して、ラボ環境で **Container Registery** を登録します。
...
7. [Cloud Shell] ウィンドウ内の Bash セッションで、次のコマンドを実行して新しい Azure Container Registry (ACR) インスタンスを作成します (ACR の名前はグローバルに一意である必要があります)。 

    ```sh
    az acr create --resource-group AZ500LAB09 --name az500$RANDOM$RANDOM --sku Basic
    ```

8. [Cloud Shell] ウィンドウ内の Bash セッションで、次の手順を実行して、新しい ACR が作成されたことを確認します。
...
3. **[Kubernetes クラスターを作成]** ブレードの **[基本]** タブで、 **[クラスターのプリセット構成]** を選び、 **[Dev/Test ($)]** を選びます。 ここで、次の設定を指定します (他の設定は既定値のままにしておきます)。
    ...
    |設定|値|
    |----|----|
    | サブスクリプション | このラボで使用している Azure サブスクリプションの名前を使用** |
    ...
    | 可用性ゾーン | **なし** |
    | AKS 価格レベル | **Free** |
    | 長期的なサポートの有効化 | **Unchecked** |
    | Kubernetes バージョン | *既定値を使用* |
    | 自動アップグレード | 既定値を維持 |
    | ノード セキュリティ チャネルの種類 | 既定値を維持** |
    | 認証と承認 | **Kubernetes RBAC を使用したローカル アカウント** |

4. **[次へ]** をクリックし **[Kubernetes クラスターを作成]** ブレードの **[ノード プール]** タブで、次の設定を指定します (他の設定は既定値のままにします)。

    |設定|Value|
    |----|----|
    | ノードの自動プロビジョニングの有効化 | **チェックボックスをオフにする** |
    | 仮想ノードを有効にする | **チェックボックスをオフにする** |
    | その他の値 | 既定値を維持** |

5. **[次へ]** をクリックして、**[ネットワーク]** に移動します。 

    >**注**: VM サイズに関する**推奨事項**を利用できることを示すポップアップが表示されます。  **推奨事項を受け入れて**、次に進みます。

6. **[Kubernetes クラスターの作成]** ブレードの **[ネットワーク]** タブで、次の設定を指定します (既定値を他のユーザーのままにします)。

    |設定|Value|
    |----|----|
    | プライベート クラスターの有効化 | **Unchecked** |
    | 承認された IP 範囲の設定 | **Unchecked** |
    | ネットワークの構成 | **Azure CNI オーバーレイ** |
...
7. **[次へ]** をクリックし、**[Kubernetes クラスターを作成]** ページの **[統合]** タブで、**すべての値を既定のまま**にします。 
...
9. デプロイが完了したら、Azure portal のページの上部にある **[リソース、サービス、ドキュメントの検索]** テキスト ボックスに「+++リソース グループ+++」と入力して **Enter** キーを押します。
...
1. [Cloud Shell] ペイン内の Bash セッションで、**[ファイルのアップロード/ダウンロード]** アイコンをクリックし、ドロップダウン メニューの **[アップロード]** をクリックして、**[開く]** ダイアログ ボックスでラボ ファイルをダウンロードした場所に移動します。**\\Allfiles\\Labs\\09\\nginxexternal.yaml** を選択し、**[開く]** をクリックします。 次に、 **\\Allfiles\\Labs\\09\\nginxinternal.yaml** を選択し、 **[開く]** をクリックします。
...
3. [Cloud Shell] ウィンドウ内の Bash セッションで、次の手順を実行して nginxexternal.yaml ファイルを開き、その内容を編集できるようにします。 
    ...
    >**注**: **[従来の Cloud Shell に切り替える]** というポップアップが表示される場合は、**[確認]** を選び、次のコマンドを実行してからこのステップを繰り返します。__
...
5. [エディター] ウィンドウの右上隅にある **省略記号**アイコンをクリックし、 **[保存]** をクリック して、 **[エディターを閉じる]** をクリックします。 
    ...
    >**注**: **Ctrl + S** キーを使ってファイルを保存し、**Ctrl + Q** キーを使ってエディターを閉じることができます。
...
7. [Cloud Shell] ウィンドウ内の Bash セッションで、前のタスクで実行したコマンドの出力を確認して、デプロイと対応するサービスが作成されたことを確認します。 次のようなページが表示されます。
...
5.  Cloud Shell ペイン内の Bash セッションで出力を調べて、デプロイとサービスが作成されたことを確認します。 次のようなページが表示されます。
...
4. **Welcome to nginx!** ページの HTML コードが 表示されることを確認します。
