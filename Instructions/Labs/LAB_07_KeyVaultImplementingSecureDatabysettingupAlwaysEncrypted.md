...
lab:
  ...
  description: SQL Server を設定し、データに対して Always Encrypted を有効にして構成します。 このラボでは、SQL Server にアクセスするクラウド環境で実行されている Web アプリケーションをシミュレートします。
  duration: 60 minutes
  level: 300
  islab: true
  primarytopics:
...
![このラボでのタスクのプロセス フローを示す図。](../media/key-vault-diagram.png)
...
3. **[カスタム デプロイ]** ブレードで、**[エディターで独自のテンプレートをビルド]** オプションをクリックします。

4. **[テンプレートの編集]** ブレードで、 **[ファイルの読み込み]** をクリックし、 **\\Allfiles\\Labs\\10\\az-500-10_baseinfra.json** ファイルを見つけて、 **[開く]** をクリックします。
...
#### az500-10-DB.json カスタム テンプレートをインストールする
...
3. **[テンプレートの編集]** ブレードで、**[ファイルの読み込み]** をクリックし、**\Allfiles\Labs\10\az-500-10_DB.json** ファイルを見つけて、**[開く]** をクリックします。
...
9. **[アクセス ポリシーの作成]** ブレードで、次の設定を指定します (他の設定はすべて既定値のままにします)。 
    ...
    |プリンシパルの選択|**[プリンシパル]** ブレードで自身のユーザー アカウントを選択して、**[次へ]** をクリックします|
...
4. **[az500-10-vm1]** ブレードに戻り、**[概要]** をクリックし、次に **[接続]** をクリックして、ドロップダウン メニューで **[接続]** をクリックします。 

5. RDP ファイルをダウンロードし、これを使用して、リモート デスクトップ経由で**az500-10-vm1** Azure VM に接続します。 認証を求めるプロンプトが表示されたら、次の認証情報を入力します。
...
12. 次のコードをクエリ ウィンドウに貼り付け、**[実行]** をクリックします。 これにより、**Patients** テーブルが作成されます。
     ...
     ```
13. テーブルが正常に作成されたら、**[オブジェクト エクスプローラー]** ペインで、**[medical]** データベース ノード、**[tables]** ノードを展開し、**[dbo.Patients]** ノードを右クリックして **[Always Encrypted ウィザード...]** をクリックします。 
...
16. **[マスター キーの構成]** ページで、**[Azure Key Vault]** を選択し、**[サインイン]** をクリックします。 このアプリのサインインのみを許可してください。 プロンプトが表示されたら、先にこのラボの Azure Key Vault インスタンスのプロビジョニングで使用したのと同じユーザー アカウントを使用して認証し、**[Azure Key Vault の選択]** ドロップダウン リストにその Key Vault が表示されるのを確認して、**[次へ]** をクリックします。
...
4. **[IE セキュリティ強化の構成]** を **[無効]** に設定します。

5. ブラウザーを開き、IE ESC がオフになっているという警告を無視します。

6. 「 https://visualstudio.microsoft.com/downloads 」を参照してください。

7. **[Visual Studio 2026]** ボックスの **[Community]** で **[無料ダウンロード]** を選択します。

8. ダウンロードが完了したら、**[ファイルを開く]** を選択します。
...
1. **az500-10-vm1** の RDP セッションで、**スタート メニュー**から **[Visual Studio 2026]** を起動します。

2. Visual Studio 2026 のウェルカム メッセージが表示されているウィンドウに切り替え、**[サインイン]** ボタンをクリックし、プロンプトが表示されたら、このラボで使用している Azure サブスクリプションへの認証に使用した資格情報を入力します。
...
11. Visual Studio ウィンドウにある **Program.cs** ペインの 15 行目で、`<接続文字列 noted earlier>` プレースホルダーを、このラボで前に記録した Azure SQL データベース **ADO.NET** の接続文字列に置き換えます。 接続文字列で、`{your_password}` のプレースホルダーを、演習 1 のデプロイで指定したパスワードに置き換えます。 ラボ コンピューターに文字列を保存した場合は、RDP セッションを終了して ADO 文字列をコピーしてから、Azure 仮想マシンに戻って貼り付ける必要がある場合があります。
